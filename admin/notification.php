...
// Handle deletion if 'delete_id' is set via GET
if (isset($_GET['delete_id'])) {
    $id = intval($_GET['delete_id']);

    // Get the PDF file to delete from server
    $stmt = $conn->prepare("SELECT pdf FROM notification WHERE id = ?");
    $stmt->bind_param("i", $id);
    $stmt->execute();
    $stmt->bind_result($pdf_file);
    if ($stmt->fetch()) {
        $file_path = __DIR__ . "/uploads/" . $pdf_file;
        if (file_exists($file_path)) {
            unlink($file_path); // Delete the PDF
        }
    }
    $stmt->close();
...
<head>
    ...
    <title>JKEIT Admin - Notifications</title>
            ...
            <div class="main-panel">
                <div class="content-wrapper">
                    <div class="page-header flex-wrap">
                        <div class="d-flex">
                            <a href="add_notification.php" class="btn btn-sm btn-success ml-3"><i class="mdi mdi-plus"></i> Add Notification</a>
                        </div>
                    </div>
                    ...
                    <div class="row">
                        <div class="col-12 grid-margin stretch-card">
                            <div class="card shadow-sm">
                                <div class="card-body">
                                    <h4 class="card-title mb-4">All Notifications</h4>
                                                ...
                                                if ($result->num_rows > 0) {
                                                    ...
                                                    while ($row = $result->fetch_assoc()) {
                                                        echo "<tr>
                                                            ...
                                                            <td data-label='PDF'>
                                                                <a href='uploads/{$row['pdf']}' target='_blank' class='btn btn-sm btn-outline-primary'>View PDF</a>
                                                            </td>
                                                            <td data-label='Title'>{$row['title']}</td>
                                                            <td data-label='Actions'>
                                                                <div class='d-flex justify-content-center gap-1'>
                                                                    <a href='edit_notification.php?id={$row['id']}' class='btn btn-sm btn-primary'><i class='mdi mdi-pencil'></i></a>   
                                                    ...
                                                    echo "<tr><td colspan='4' class='text-center text-muted'>No notifications found</td></tr>";
                                                }
                                                ?>
                                            </tbody>
                                        </table>
            ...
            btn.addEventListener('click', function () {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        // Redirect with delete_id in URL
                        window.location.href = "notification.php?delete_id=" + id;
                    }
                });
            });
        });
