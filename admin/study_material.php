...
<head>
    ...
    <title>JKEIT Admin - syllabus</title>
            ...
            <div class="main-panel">
                <div class="content-wrapper">
                    <div class="page-header flex-wrap">
                        <h3 class="mb-0">All study Material</h3>
                    </div>
                                            ...
                                            <tbody>
                                                ...
                                                if ($result && $result->num_rows > 0) {
                                                    ...
                                                    while ($row = $result->fetch_assoc()) {
                                                        ...
                                                        // Fetch PDF(s) for this subject
                                                        $pdf_sql = "SELECT id, pdf_file FROM study_pdf WHERE subject_name='" . $conn->real_escape_string($row['subject_name']) . "'";
                                                        $pdf_result = $conn->query($pdf_sql);

                                                        if ($pdf_result && $pdf_result->num_rows > 0) {
                                                            echo "<td data-label='Subject PDF'>";
                                                            $first_pdf_id = null; // To use for edit button
                                                            while ($pdf_row = $pdf_result->fetch_assoc()) {
                                                                if (!$first_pdf_id) {
                                                                    $first_pdf_id = $pdf_row['id'];
                                                                }
                                                                echo "<a href='uploads/pdfs/" . htmlspecialchars($pdf_row['pdf_file']) . "' target='_blank' class='btn btn-sm btn-info mb-1'>
                                                            ...
                                                            echo "<td data-label='Action'>
                                                                    <a href='add_study_meterial.php?id=" . $row['id'] . "' class='btn btn-sm   btn-success' title='Add PDF'>
                                                                    <i class='fa fa-plus'></i>
                                                                    </a>
                                                                    <a href='edit_study_material.php?id=" . $first_pdf_id . "' class='btn btn-sm btn-primary' title='Edit PDF'>
                                                                    <i class='fa fa-edit'></i>
                                                                     </a>
                                                                  <a href='delete_study_material.php?id=" . $first_pdf_id . "' 
                                                            ...
                                                            // No PDF
                                                            echo "<td data-label='Subject PDF'><span class='text-muted'>No PDF</span></td>";
                                                            echo "<td data-label='Action'>
                                                                  <a href='add_study_meterial.php?id=" . $row['id'] . "' class='btn btn-sm btn-success' title='Add PDF'>
                                                                    <i class='fa fa-plus'></i>
                                                                     </a>
                                                                       <span class='btn btn-sm btn-primary disabled' title='No PDF to edit'>
                                                                         <i class='fa fa-edit'></i>
                                                                             </span>
                                                                           <a href='delete_study_material.php?id=" . $row['id'] . "' 
                                                                            class='btn btn-sm btn-danger' 
                                                                                onclick=\"return confirm('Are you sure you want to delete this PDF?');\">
                                                                            <i class='fa fa-trash'></i>
                                                                             </a>
