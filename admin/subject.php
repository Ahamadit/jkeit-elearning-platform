                                            ...
                                            <tbody>
                                                ...
                                                if ($result->num_rows > 0) {
                                                    ...
                                                    while ($row = $result->fetch_assoc()) {
                                                        $course_id = $row['id']; // course id from add_course

                                                        // fetch all subjects for this course
                                                        $subject_sql = "SELECT subject_name FROM add_subject WHERE course_id = '$course_id'";
                                                        $subject_result = $conn->query($subject_sql);

                                                        $subject_list = "";
                                                        if ($subject_result->num_rows > 0) {
                                                            while ($sub = $subject_result->fetch_assoc()) {
                                                                $subject_list .= $sub['subject_name'] . ", ";
                                                            }
                                                            $subject_list = rtrim($subject_list, ", "); // remove last comma
                                                        } else {
                                                            $subject_list = "No subjects added";
                                                        }

                                                        echo "<tr>
                <td>{$sr}</td>
                <td>{$row['course_name']}</td>
                <td>" . htmlspecialchars($subject_list) . "</td>
                <td>
                    <a href='add_subject.php?course_id={$row['id']}' 
                       ...
                       <i class='mdi mdi-book-plus'></i>
                                                    ...
                                                    echo "<tr><td colspan='4' class='text-center text-muted'>No courses found</td></tr>";
                                                }
                                                ?>
                                            </tbody>
