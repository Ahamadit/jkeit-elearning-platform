...
if (!isset($_GET['id'])) {
    echo "<script>alert('Invalid request!'); window.location.href='vision.php';</script>";
    exit();
}
...
$query = "SELECT * FROM vision WHERE id = ?";
...
if ($result->num_rows === 0) {
    echo "<script>alert('Vision not found!'); window.location.href='vision.php';</script>";
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($_FILES['image']['name'])) {
        $uploadDir = __DIR__ . '/uploads/vision/';
        if (!is_dir($uploadDir)) {
    ...
    $updateQuery = "UPDATE vision SET image = ?, heading = ?, paragraph = ? WHERE id = ?";
    $stmt = $conn->prepare($updateQuery);
    $stmt->bind_param("sssi", $newImageName, $heading, $paragraph, $id);

    if ($stmt->execute()) {
        echo "<script>alert('Vision updated successfully!'); window.location.href='vision.php';</script>";
    } else {
...
<body class="bg-gray-100 min-h-screen flex items-center justify-center">
  ...
  <div class="w-full max-w-lg bg-white shadow-lg rounded-2xl p-8">
    <h2 class="text-2xl font-bold text-gray-800 mb-6 text-center">Edit Vision</h2>
