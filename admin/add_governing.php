...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("INSERT INTO governing (name, role, details) VALUES (?, ?, ?)");
    $stmt->bind_param("sss", $name, $role, $details);

    if ($stmt->execute()) {
        echo "<script>
                alert('Team Governing council added successfully!');
...
<body class="bg-gray-100 min-h-screen flex items-center justify-center">
  ...
  <div class="w-full max-w-2xl bg-white shadow-lg rounded-2xl p-8">
    <h2 class="text-3xl font-bold text-gray-800 mb-8 text-center">Add Governing council</h2>
