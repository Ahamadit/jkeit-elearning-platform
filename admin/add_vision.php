...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $uploadDir = __DIR__ . '/uploads/vision/'; // Server folder
    ...
    if (move_uploaded_file($imageTmp, $targetFile)) {
        // Save to database
        $sql = "INSERT INTO vision (image, heading, paragraph) VALUES (?, ?, ?)";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("sss", $filename, $heading, $paragraph);

        if ($stmt->execute()) {
            echo "<script>alert('Vision added successfully!'); window.location.href='vision.php';</script>";
        } else {
...
<head>
  ...
  <title>Add New Vision</title>
  <script src="https://cdn.tailwindcss.com"></script>
</head>
<body class="bg-gray-100 min-h-screen flex items-center justify-center">

  <div class="w-full max-w-lg bg-white shadow-lg rounded-2xl p-8">
    <h2 class="text-2xl font-bold text-gray-800 mb-6 text-center">Add New Vision</h2>
