...
$stmt = $conn->prepare("SELECT * FROM governing WHERE id = ?");
$stmt->bind_param("i", $id);
$stmt->execute();
$result = $stmt->get_result();

if ($result->num_rows === 0) {
    echo "<script>alert('governing council not found!'); window.location.href='governing.php';</script>";
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $update = $conn->prepare("UPDATE governing SET name = ?, role = ?, details = ? WHERE id = ?");
    $update->bind_param("sssi", $name, $role, $details, $id);

    if ($update->execute()) {
        echo "<script>
                alert('Governing Council updated successfully!');
...
<head>
  ...
  <title>Edit Governing Council</title>
  <script src="https://cdn.tailwindcss.com"></script>
</head>
<body class="bg-gray-100 min-h-screen flex items-center justify-center">

  <div class="w-full max-w-2xl bg-white shadow-lg rounded-2xl p-8">
    <h2 class="text-3xl font-bold text-gray-800 mb-8 text-center">Edit Governing Council</h2>
