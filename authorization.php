...
// Fetch all authorization records
...
<head>
    ...
    <style>
        ...
        .social-links a { background: rgba(255,255,255,0.1); padding: 8px; border-radius: 50%; font-size: 16px; display: inline-flex; align-items: center; justify-content: center; transition: background 0.3s ease, transform 0.3s ease; }
        .social-links a:hover { background: #fff; color: #37423B !important; transform: translateY(-4px); }
        #preloader { position: fixed; top:0; left:0; right:0; bottom:0; background: #fff url("assets/img/loader.gif") no-repeat center center; z-index: 9999; }

        /* Hero Section */
        .hero { position: relative; width: 100%; height: 60vh; max-height: 400px; min-height: 200px; overflow: hidden; display: flex; align-items: center; justify-content: center; color: #fff; text-align: center; margin:0; padding:0; }
        ...
        .hero-content h2, .hero-content p { margin:0.3em 0; line-height:1.3; }
        @media (max-width:768px) { .hero { height:25vh; min-height:150px; } }

        /* Cards */
        .vision-mission .card { border:none; transition: all 0.4s ease-in-out; background:#fff; }
        .vision-mission .card:hover { background: linear-gradient(135deg, #0077b6, #00b4d8); color:#fff; transform:translateY(-8px); box-shadow:0px 8px 20px rgba(0,0,0,0.15); }
        .vision-mission .card:hover i, .vision-mission .card:hover h4, .vision-mission .card:hover p { color:#fff !important; }
        @media (max-width:640px) { section header { flex-direction:column; text-align:center; } section header div { margin:4px 0; } }
    ...
    <section style="background:#023E8A; padding:8px 20px;">
        <header style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap;">
            <div style="margin:5px 0;">
                <a href="tel:+919634599903" style="text-decoration:none;color:#fff;font-size:1rem;font-weight:500;display:flex;align-items:center;">
                    <i class="fa-solid fa-phone" style="margin-right:8px;color:#FFD60A;"></i> +91 9634599903
                </a>
            </div>
            <div style="margin:5px 0;">
                <a href="mailto:jkeitonline@gmail.com" style="text-decoration:none;color:#fff;font-size:1rem;display:flex;align-items:center;">
                    <i class="fa-solid fa-envelope" style="margin-right:6px;color:#FFD60A;"></i> jkeitonline@gmail.com
                </a>
            </div>
        </header>
    </section>
    ...
    <header id="header" class="header d-flex align-items-center bg-white shadow-sm">
        <div class="container-fluid container-xl position-relative d-flex align-items-center justify-content-between">
            <div style="display:flex;align-items:center;gap:20px;">
                <div>
                    <img src="assets/logo/logo2.jpeg" alt="JAN KALYAN EDUCATION INSTITUTE Logo" style="width:250px;height:auto;object-fit:contain;">
                </div>
            </div>
...
<main class="main">
    ...
    <section id="hero" class="hero section dark-background">
        <img src="assets/slider/slider2.avif" alt="JKEIT Authorization" class="hero-img" data-aos="fade-in">
        <div class="container hero-content text-center">
            <h2 data-aos="fade-up" data-aos-delay="100">AUTHORIZATION - JKEIT</h2>
            <p data-aos="fade-up" data-aos-delay="200">Authorized and recognized by relevant authorities, JKEIT operates with full compliance to ensure quality education and transparent governance.</p>
        </div>
    </section>

    <!-- Authorization Section -->
    <section id="authorization" class="authorization section py-5 bg-light">
        <div class="container" data-aos="fade-up">
            <div class="section-title text-center mb-5">
                <h2 class="fw-bold">Our Authorization</h2>
                ...
                <div class="row gy-4">
                    ...
                    if (mysqli_num_rows($result) > 0) {
                        while ($row = mysqli_fetch_assoc($result)) {
                            ...
                            $paragraph = nl2br(htmlspecialchars($row['paragraph']));
                            $imageFile = trim($row['image']);
                            $imagePath = "admin/uploads/authorization/" . $imageFile;

                            if (!empty($imageFile)) {
                                $imgTag = "<img src='{$imagePath}' alt='Authorization Image'>";
                            } else {
                                $imgTag = "<img src='assets/images/no-image.png' alt='No Image'>";
                            }
                    ?>
                            <div class="col-lg-4 col-md-6" data-aos="fade-up">
                                <div class="card shadow-sm p-4 rounded-3 h-100 text-center">
                                    <div class="mb-3">
                                        <?= $imgTag ?>
                                    </div>
                                    <h4 class="fw-bold"><?= $heading ?></h4>
                                    <p class="text-muted mt-3"><?= $paragraph ?></p>
                                </div>
                            </div>
                    <?php
                        }
                    } else {
                        echo "<div class='col-12 text-center'><p class='text-muted'>No authorization records found.</p></div>";
                    }
                    ?>
                </div>
            </div>
