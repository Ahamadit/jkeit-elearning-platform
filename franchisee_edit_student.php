...
if (!isset($_GET['id']) || empty($_GET['id'])) {
    header("Location: franchisee_student.php");
    exit;
}
...
// Fetch student data
$stmt = $conn->prepare("SELECT * FROM franchisee_students WHERE id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $course = trim($_POST['course']); // new field

    if ($student_name && $email && $password && $mobile && $course) {
        $stmt = $conn->prepare("UPDATE franchisee_students SET name=?, email=?, password=?, mobile=?, course=? WHERE id=?");
        $stmt->bind_param("sssssi", $student_name, $email, $password, $mobile, $course, $student_id);
        if ($stmt->execute()) {
            $show_sweetalert = true; // trigger SweetAlert
            // Update $student variable to show updated values in the form
...
<title>Edit Student | Franchisee Panel</title>
...
<body>
<div class="form-card">
    <h3 class="text-center mb-3"><i class="fa-solid fa-user-pen me-2"></i>Edit Student</h3>
    ...
    <form method="POST" action="">
        ...
        <div class="mb-3">
            ...
            <input type="text" name="student_name" id="student_name" class="form-control" value="<?= htmlspecialchars($student['name']) ?>" required>
        </div>
        ...
        <div class="mb-3">
            ...
            <input type="text" name="password" id="password" class="form-control" value="<?= htmlspecialchars($student['password']) ?>" required>
        </div>

        <div class="mb-3">
            <label for="mobile" class="form-label"><i class="fa-solid fa-phone me-1"></i> Mobile Number</label>
            <input type="text" name="mobile" id="mobile" class="form-control" value="<?= htmlspecialchars($student['mobile']) ?>" required pattern="[0-9]{10}" maxlength="10">
        </div>

        <!-- New Course Field -->
        <div class="mb-3">
            <label for="course" class="form-label"><i class="fa-solid fa-book me-1"></i> Course</label>
            <input type="text" name="course" id="course" class="form-control" value="<?= htmlspecialchars($student['course']) ?>" required>
        </div>

        <div class="d-grid">
            <button type="submit" class="btn btn-custom"><i class="fa-solid fa-paper-plane me-1"></i> Update</button>
        </div>
