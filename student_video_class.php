...
// Fetch videos
$videos = [];
$stmt3 = $conn->prepare("SELECT subject_name, video_link FROM video_class WHERE subject_name IN (
...
while ($row = $res3->fetch_assoc()) {
    $videos[$row['subject_name']][] = $row['video_link'];
}
$stmt3->close();
?>
<!doctype html>
...
<head>
  ...
  <title>Student Video Class</title>
...
<div class="flex pt-16">
  ...
  <aside class="w-64 bg-gradient-to-b from-blue-600 to-blue-800 text-white h-screen p-5 fixed">
    <ul class="space-y-4">
      ...
      <li><a href="student_video_class.php" class="flex items-center space-x-3 bg-blue-700 p-2 rounded-lg"><i data-feather="video"></i><span>Video Class</span></a></li>
  ...
  <main class="ml-64 flex-1 p-8">
    <h1 class="text-2xl font-bold mb-6">Video Classes for <?php echo htmlspecialchars($courseName); ?></h1>
        ...
        <table class="min-w-full bg-white rounded shadow">
          <thead>
            <tr class="bg-blue-600 text-white">
              ...
              <th class="py-3 px-4 text-left">Video Links</th>
            </tr>
          </thead>
          <tbody>
            <?php $sr = 1; ?>
            <?php foreach ($subjects as $id => $subject_name): ?>
              <tr class="border-b hover:bg-gray-100">
                ...
                <td class="py-3 px-4">
                  <?php if (!empty($videos[$subject_name])): ?>
                    <?php foreach ($videos[$subject_name] as $link): ?>
                      <a href="<?php echo htmlspecialchars($link); ?>" target="_blank" class="inline-block bg-green-500 text-white px-3 py-1 rounded mb-1 hover:bg-green-600">View Video</a>
                    <?php endforeach; ?>
                  <?php else: ?>
                    <span class="text-gray-500">No videos available</span>
                  <?php endif; ?>
                </td>
              </tr>
            <?php endforeach; ?>
